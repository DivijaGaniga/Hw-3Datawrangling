...
x <- county_MI %>% mutate(total_employement = (Professional+Production+Service+Office+Construction)) %>% 
                           select(County,Professional,Production,Service,Office,Construction,total_employement)

head(x$total_employement > 100.105 | x$total_employement < 99.895 )
...
```{r summarystats_mi}
 county_MI %>% group_by(peninsula) %>%
              ...
              rangeTotalPop('min-max values') = max(TotalPop) - min(TotalPop),
